...
import { SALAD, SALAD_BNB_FARM, SALAD_BNB_TOKEN, SALAD_TOKEN, MASTERCHEF_CONTRACT, WBNB_TOKEN } from "./constants";
import { Pair, TokenAmount } from "@saladswap/sdk";
...
const pools: string[] = [
  "0xF1bd5673Ea4a1C415ec84fa3E402F2F7788E7717", // DUSK
  "0xB4C68A1C565298834360BbFF1652284275120D47", // BIFI
...
export const getTotalStaked = async (address: string, block: string): Promise<number> => {
  ...
  try {
    // Salad balance in wallet.
    const saladContract = getContract(bep20ABI, SALAD, true);
    const saladBalance = await saladContract.methods.balanceOf(address).call(undefined, blockNumber);
    balance = balance.plus(saladBalance);
  } catch (error) {
    console.error(`SALAD balance error: ${error}`);
  }

  try {
    // SALAD-BNB farm.
    const saladBnbContract = getContract(pairABI, SALAD_BNB_FARM, true);
    const totalSupplyLP = await saladBnbContract.methods.totalSupply().call(undefined, blockNumber);
    const reservesLP = await saladBnbContract.methods.getReserves().call(undefined, blockNumber);
    const saladBnbBalance = await saladBnbContract.methods.balanceOf(address).call(undefined, blockNumber);
    const pair: Pair = new Pair(
      new TokenAmount(SALAD_TOKEN, reservesLP._reserve0.toString()),
      new TokenAmount(WBNB_TOKEN, reservesLP._reserve1.toString())
    );
    const saladLPBalance = pair.getLiquidityValue(
      pair.token0,
      new TokenAmount(SALAD_BNB_TOKEN, totalSupplyLP.toString()),
      new TokenAmount(SALAD_BNB_TOKEN, saladBnbBalance.toString()),
      false
    );
    balance = balance.plus(new BigNumber(saladLPBalance.toSignificant(18)).times(1e18));
  } catch (error) {
    console.error(`SALAD-BNB LP error: ${error}`);
  }

  try {
    // MasterChef contract.
    const masterContract = getContract(masterChefABI, MASTERCHEF_CONTRACT, true);
    const saladMainStaking: UserInfoResult = await masterContract.methods
      .userInfo(0, address)
      .call(undefined, blockNumber);
    balance = balance.plus(saladMainStaking.amount);
  } catch (error) {
